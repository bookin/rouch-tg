...
def create_problem_agent() -> Agent[ProblemContext, ProblemSolution]:
    ...
    agent = Agent(
        ...
        system_prompt=(
            ...
            "   - Сгенерируй `partner_selection_guide` для 4 универсальных категорий партнеров, специфично адаптированных под проблему пользователя.\n"
            "     Партнер — это «почва» для кармического семени. Подбирай партнеров так, чтобы действия для них были в той же сфере, что и проблема.\n"
            "\n"
            "     1. Source (Источник) — те, кто дает ресурсы, фундамент, поддержку.\n"
            "        Универсальные примеры: Родители, учителя, наставники, врачи, авторы книг, которые вдохновили.\n"
            "        В `description` и `examples` выбирай наиболее вероятные источники именно для этой сферы и ситуации пользователя, не формально.\n"
            "        Ключевой вопрос: «Кому я благодарен за то, что имею сейчас?»\n"
            "        Задача пользователя: Проявлять благодарность, заботу и уважение. Возвращать внимание и поддержку.\n"
            "        Кармический результат: Стабильность, ресурсы, защита от кризисов.\n"
            "\n"
            "     2. Ally (Соратник) — те, кто на одном уровне, идут к общей цели.\n"
            "        Универсальные примеры: Супруг/а, близкие друзья, братья/сестры, единомышленники, соседи.\n"
            "        В `examples` указывай реальные группы из окружения пользователя. Если изоляция — онлайн-соратники, сообщества.\n"
            "        Ключевой вопрос: «Кто помогает мне тащить воз ежедневной жизни?»\n"
            "        Задача пользователя: Делать их героями — помогать чувствовать себя ценными и способными, снимать груз.\n"
            "        Кармический результат: Надежные отношения, помощь в нужный момент, отсутствие одиночества.\n"
            "\n"
            "     3. Protege (Подопечный) — те, кто нуждается в помощи, зависит от пользователя.\n"
            "        Универсальные примеры: Дети, ученики, животные, люди, которым помогаешь советом, аудитория.\n"
            "        Подбирай подопечных так, чтобы им реально можно было помочь в этой сфере малыми конкретными действиями.\n"
            "        Ключевой вопрос: «Чью жизнь я могу сделать лучше прямо сейчас?»\n"
            "        Задача пользователя: Искреннее служение — давать больше, чем ожидают, заботиться как мать о ребенке.\n"
            "        Кармический результат: Рост, признание, поток входящей энергии (деньги, любовь), востребованность.\n"
            "\n"
            "     4. World (Внешний мир) — далекие люди, незнакомцы, конкуренты, те, кто неприятен.\n"
            "        Универсальные примеры: Случайные прохожие, общество, конкуренты в нише, люди, которые мешают.\n"
            "        Описывай эту группу без морализаторства — как поле практики, а не урок морали.\n"
            "        Ключевой вопрос: «Как я отношусь к тем, от кого мне (казалось бы) нет пользы?»\n"
            "        Задача пользователя: Желать им успеха, отказаться от зависти, делать тайные добрые дела.\n"
            "        Кармический результат: Устранение внутренних блоков, удача в неожиданных ситуациях, масштаб личности.\n"
            "\n"
            "     Для КАЖДОЙ из 4 категорий обязательно заполни `fallback_advice` — что делать, если у человека НЕТ никого в этой группе.\n"
            "     В случае изоляции предлагай конкретные альтернативы:\n"
            "       - Ментальные практики: пожелание счастья, визуализация благополучия этой группы людей.\n"
            "       - Онлайн-форматы: волонтерство, помощь на форумах, донаты, поддержка в сообществах.\n"
            "       - Анонимные действия: уборка мусора, анонимный донат, добрый отзыв незнакомцу.\n"
            "\nОформление ответа:\n"

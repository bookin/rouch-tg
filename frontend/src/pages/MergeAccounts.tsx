		...
		<Card className="max-w-lg mx-auto relative">
			<CardContent>
				<div className="space-y-5">
					...
					<section className="rounded-xl bg-white/10 border border-white/30 p-4 space-y-3">
					...
					{has_project_conflict && (
						<section className="rounded-xl bg-amber-900/20 border border-amber-600/30 p-4 space-y-3">
							...
							<p className="text-xs text-muted-foreground">
								Выбери, какой проект оставить активным. Второй будет приостановлен — его можно будет
								возобновить
								позже.
							</p>
							<div className="space-y-2">
								<label
									className="flex items-start gap-2 cursor-pointer rounded-lg p-2 hover:bg-white/5 transition-colors">
									<input
										type="radio"
										name="keepProject"
            ...
            Понимаю — хочу объединить аккаунты
					...
					{error && (
						<p className="text-sm text-red-300 bg-red-900/20 rounded-sm px-3 py-2">{error}</p>
					)}
...
function AccountCard({
	...
	const borderColor = accent === 'emerald' ? 'border-emerald-600' : 'border-amber-600'

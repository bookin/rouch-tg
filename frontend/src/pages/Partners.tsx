	...
	if (loading && groups.length === 0) {
		return (
			<div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
				...
				<p className=" text-sm">Загружаем твоё окружение...</p>
			</div>
		)
	}
	...
	return (
		<div className="flex flex-col gap-6 p-4 max-w-5xl mx-auto w-full pb-24">
			<div className="space-y-1 mt-2">
				<PageHeader text="Партнёры" icon={Users}/>
				...
				{CATEGORY_ORDER.map(cat => {
					...
					return (
						<Card key={cat}
							...
							<CardHeader className="pb-3 pt-5 px-5 bg-white/20 border-b border-white/10">
								<div className="flex items-center gap-3">
									<div className={cn(
										"h-10 w-10 shrink-0 aspect-square rounded-full flex items-center justify-center text-lg shadow-sm border border-white/20",
										CATEGORY_COLORS[cat]
									)}>
										{groupInfo?.icon || '👤'}
									</div>
							...
							<CardContent className="px-5 pb-5 pt-0 mt-4">
								<div className="grid gap-2">
									{catPartners.length === 0 && (
										<div
											className="text-sm/12 italic py-2 text-center bg-white/10 rounded-lg border border-dashed border-white/30">
									...
									{catPartners.map(p => (
										<div
											key={p.id}
											className="relative flex items-center justify-between p-3 rounded-lg bg-white/10 border border-white/30 hover:border-white/40 transition-all shadow-sm hover:bg-white/25"
										>
											...
											{p.notes && (
												<Popover>
													...
													<PopoverPortal>
														<PopoverContent>
															<div className="w-60 p-3 bg-white/20 backdrop-blur-xl text-xs rounded-md shadow-lg border border-white/40 text-white">
															<div className="font-semibold mb-1">Заметки:</div>
															{p.notes}
															</div>
														</PopoverContent>
													</PopoverPortal>
													...
													{/*{activeNoteId === p.id && (*/}
													{/*	<div*/}
													{/*		className="absolute right-0 top-12 z-20 w-48 p-3 bg-white/90 backdrop-blur-xl text-foreground text-xs rounded-md shadow-lg border border-white/40 animate-in fade-in zoom-in-95 duration-200">*/}
													{/*		<div className="font-semibold mb-1 text-primary">Заметки:*/}
													{/*		</div>*/}
													{/*		{p.notes}*/}
													{/*	</div>*/}
													{/*)}*/}
			...
			{/* Add Partner Form */}
			<Card>
				<CardHeader className="bg-secondary/30 border-b border-white/10">
				...
				<CardContent className="pt-5 space-y-4">
					<form onSubmit={onCreatePartner} className="space-y-4">
						...
						<div className="grid grid-cols-2 gap-3">
							<Button
								type="button"
								variant={newPartnerContactType === 'physical' ? 'default' : 'glass'}
								// size="sm"
							...
							<Button
								type="button"
								variant={newPartnerContactType === 'online' ? 'default' : 'glass'}
								// size="sm"
